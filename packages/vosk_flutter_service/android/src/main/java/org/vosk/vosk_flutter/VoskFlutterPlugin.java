...
public class VoskFlutterPlugin implements FlutterPlugin, MethodCallHandler {
    ...
    synchronized (lifecycleLock) {
      ...
      try {
        ...
        case "recognizer.acceptWaveForm": {
          ...
          boolean accepted;
          if (bytes == null) {
            accepted = recognizer.acceptWaveForm(floats, floats.length);
          } else {
            accepted = recognizer.acceptWaveForm(bytes, bytes.length);
          }
          long elapsedMs = SystemClock.elapsedRealtime() - startedAt;
          if (elapsedMs >= 150) {
            Log.w(TAG, "slow acceptWaveForm recognizerId=" + recognizerId
                + " bytes=" + (bytes == null ? 0 : bytes.length)
                + " floats=" + (floats == null ? 0 : floats.length)
                + " nativeMs=" + elapsedMs
  ...
  public void onDetachedFromEngine(@NonNull FlutterPluginBinding binding) {
    synchronized (lifecycleLock) {
      ...
      for (SpeakerModel speakerModel : speakerModelsMap.values()) {
        speakerModel.close();
      }
