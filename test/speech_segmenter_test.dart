...
void main() {
  test('both lanes can share the same segment identity until silence endpoint',
      () {
    final SpeechSegmenter segmenter = SpeechSegmenter(endpointSilenceChunks: 2);
    segmenter.begin(3);

    final SpeechSegment first =
        segmenter.add(Uint8List.fromList(<int>[0, 16]), 3)!;
  ...
  test('new capture epoch discards old chunks and restarts segment numbering',
    ...
    segmenter.begin(1);
    segmenter.add(Uint8List.fromList(<int>[0, 16]), 1);
    segmenter.begin(2);

    expect(segmenter.add(Uint8List.fromList(<int>[0, 16]), 1), isNull);
    final SpeechSegment fresh =
        segmenter.add(Uint8List.fromList(<int>[0, 16]), 2)!;
