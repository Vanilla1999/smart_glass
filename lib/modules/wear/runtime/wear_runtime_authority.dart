...
class WearSessionSlice {
  const WearSessionSlice._(this.user);
  ...
  WearReduction reduce(WearRuntimeState state, WearIntent intent) {
    ...
    if (intent is WearSessionAuthorized) {
      if (payload.session.isAuthorized) {
        if (payload.session.sameIdentityAs(
      ...
      return WearReduction.accept(
        nextState: state.withPayload(
          payload.copyWith(
            session: WearSessionSlice.authorized(intent.user),
    ...
    if (intent is WearLogicalNavigationRequested) {
      if (payload.navigation.logicalScreen == intent.screen &&
          payload.navigation.pending == null) {
        return WearReduction.accept();
      }
    ...
    if (intent is WearNavigationAcknowledged) {
      ...
      return WearReduction.accept(
        nextState: state.withPayload(
          payload.copyWith(
            navigation: payload.navigation.acknowledge(
...
class WearRuntimeAuthority {
  ...
  Future<WearDispatchResult> authorize(AuthenticatedUser user) async {
    ...
    if (result.accepted && payload.session.user != null) {
  ...
  Future<WearDispatchResult> clearSession() async {
    ...
    if (result.accepted && wasAuthorized && !_cleared.isClosed) {
      _cleared.add(null);
    }
    return result;
  }
  ...
  Future<WearDispatchResult> terminate() {
    return _store.dispatch(const WearRuntimeTerminated());
  }

  Future<void> dispose() async {
    await _store.dispose();
    await _authorized.close();
    await _cleared.close();
  }
}
