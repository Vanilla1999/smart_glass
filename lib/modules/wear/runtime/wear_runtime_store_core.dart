...
/// Read-only view of values that are still owned by the legacy runtime.
...
///
/// Feature state is deliberately absent: existing feature runtimes remain the
/// only writable owners until their dedicated migration MR.
class WearRuntimeState {
  ...
  WearRuntimeState expectOperation({
    ...
    if (normalizedKind.isEmpty) {
      throw ArgumentError.value(kind, 'kind', 'Operation kind must not be empty');
    }
    if (operationId < 0) {
    ...
    try {
      while (_queue.isNotEmpty && !_terminalBarrier) {
        ...
        try {
          ...
          // Validate the complete transition before publishing anything.
